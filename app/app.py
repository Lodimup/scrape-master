...
def scrape_multiple_urls(urls, fields, selected_model):
    output_folder = os.path.join(
        "output", generate_unique_folder_name(urls[0]))
    ...
    for i, url in enumerate(urls, start=1):
        ...
        input_tokens, output_tokens, cost, formatted_data = scrape_url(
            url, fields, selected_model, output_folder, i, markdown
        )
...
def perform_scrape():
    ...
    if show_tags:
        ...
        input_tokens, output_tokens, total_cost = calculate_price(
            tokens_count, model=model_selection
        )
...
if st.sidebar.button("Scrape"):
    with st.spinner("Please wait... Data is being scraped."):
        ...
        (
            ...
            all_data,
            first_url_markdown,
        ) = scrape_multiple_urls(urls, field_list, model_selection)
        ...
        if use_pagination and len(urls) == 1:
            try:
                ...
                if pagination_result is not None:
                    ...
                    st.warning(
                        "Pagination detection returned None. No pagination information available."
                    )
...
if st.session_state["results"]:
    ...
    if pagination_info and use_pagination:
        ...
        st.sidebar.markdown(
            f"*Input Tokens:* {pagination_info['token_counts']
                               ['input_tokens']}"
        )
        st.sidebar.markdown(
            f"*Output Tokens:* {pagination_info['token_counts']
                                ['output_tokens']}"
        )
        st.sidebar.markdown(
            f"**Pagination Cost:** :red-background[**${
                pagination_info['price']:.4f}**]"
        )

        st.markdown("---")
        st.subheader("Pagination Information")
        pagination_df = pd.DataFrame(
            pagination_info["page_urls"], columns=["Page URLs"]
        )
    ...
    if all_data and pagination_info and show_tags and use_pagination:
        st.markdown("---")
        total_input_tokens = (
            input_tokens + pagination_info["token_counts"]["input_tokens"]
        )
        total_output_tokens = (
            output_tokens + pagination_info["token_counts"]["output_tokens"]
        )
        ...
        st.markdown(
            f"**Total Combined Cost:** :green[**${total_combined_cost:.4f}**]")
